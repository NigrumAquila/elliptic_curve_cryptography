...
EllipticCurve::Point* ElGamal::encrypt(EllipticCurve::Point& data, EllipticCurve::Point& publicKey)
{
	...
	cipher[1] = this->ec.addiction(data, this->ec.multiply(publicKey, random));
	return cipher;
}

EllipticCurve::Point ElGamal::decrypt(EllipticCurve::Point* cipher, __int64 privateKey)
{
	EllipticCurve::Point res = this->ec.addiction(cipher[1], this->ec.reflect(this->ec.multiply(cipher[0], privateKey)));
	return res;
}
